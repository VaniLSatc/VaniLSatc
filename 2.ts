...
function searchCycle(str: string, clmn: number): number {
...
let rwbColumns = -1; // Переменная отвечает за номер стержня, на котором находятся все цвета
const output = new Set<number>(); // Используем Set для хранения уникальных значений
const rings = prompt("Введите строку: ") || "";
// Примеры строк:
// R0R6B0W6W0W6B9
// R0W0B0W9W0R0B0
// B4
// W0R3W7B0R2W9B5R1W4B8R6W0B3R9W2B7R5W1B4R8W6B2R0W3B9R1W6
...
for (let i = 0; i < rings.length - 1; i++) {
    if ((rings[i] === 'R' || rings[i] === 'W' || rings[i] === 'B') && !isNaN(parseInt(rings[i+1]))) {
        ...
        const result = searchCycle(rings, column);
        if (result !== -1) {
            rwbColumns = result;
            output.add(rwbColumns); // Используем Set, чтобы сохранять только уникальные значения
        }
    }
}
